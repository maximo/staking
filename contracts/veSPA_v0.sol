...
contract veSPA_v0 is Ownable, ReentrancyGuard {
    ...
    int128 public constant iYEAR = 365 * 86400;
    int128 public constant iMIN_TIME = 7 * 86400;
    ...
    function _checkpoint(
        ...
        if (addr != ZERO_ADDRESS) {
            ...
            if (oldDeposit.amount > 0) {
                if (!oldDeposit.cooldownFlag) {
                    uOld.residue = (oldDeposit.amount * iMIN_TIME) / iYEAR;
                    oldDeposit.end -= WEEK;
                }
                if (oldDeposit.end > block.timestamp) {
                    uOld.slope = oldDeposit.amount / iYEAR;
            ...
            if ((newDeposit.end > block.timestamp) && (newDeposit.amount > 0)) {
                uNew.slope = newDeposit.amount / iYEAR;
                if (!newDeposit.cooldownFlag) {
                    uNew.residue = (newDeposit.amount * iMIN_TIME) / iYEAR;
                    newDeposit.end -= WEEK;
                }
                uNew.bias =
                    uNew.slope *
    ...
    function _depositFor(
        ...
        newDeposit.cooldownFlag = enableCooldown;
        if (unlockTime != 0) {
    ...
    function initiateCooldown() external nonReentrant {
        address account = _msgSender();
        LockedBalance memory existingDeposit = locked[account];
        require(
        ...
        require(existingDeposit.amount > 0, "No existing lock found");
        require(!existingDeposit.cooldownFlag, "Cooldown already initiated");
    ...
    function withdraw() external nonReentrant {
        address account = _msgSender();
        LockedBalance memory existingDeposit = locked[account];
        require(existingDeposit.cooldownFlag, "No cooldown initiated");
